...
function getcompetitive() {
    ...
    fetch(requestData.url, requestData.data).then(r => r.json()).then(r => {
        ...
        let filteredEvents = r.events.filter(event => {
        ...
        for (let event of filteredEvents) { //r.events.slice(-24)) { -- dont use this!
            ...
            line1.innerText = event.name_line1;
            let line2 = gne('h2');
            line2.classList.add('event-text-line2');
            line2.innerText = event.name_line2;

            let line3 = gne('p');
            line3.classList.add('event-text-line2');
            line3.innerText = event.region + ' - ' + event.schedule;

            let line4 = gne('h3');
            line4.classList.add('event-text-line2');
            line4.innerText = event.short_description;
